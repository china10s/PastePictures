...
- (void)Seleted:(NSString*)strDistrictName strSceneName:(NSString*)strSceneName{
    ...
    [self ReplaceControllor:(ViewController*)_vcSelected toCont:(ViewController*)_vcPictures finishedBlock:^(){
...
//图片view切换到选择view
- (void)PicSwitchBack{
    [self ReplaceControllor:(ViewController*)_vcPictures toCont:(ViewController*)_vcSelected finishedBlock:nil] ;
}

//转换view
- (void)ReplaceControllor:(ViewController*)fromCont toCont:(ViewController*)toCont
            finishedBlock:(void (^) (void) )finishedBlock {
    [self transitionFromViewController:fromCont toViewController:toCont duration:0.7 options:UIViewAnimationOptionTransitionFlipFromRight animations:nil completion:^(BOOL finished){
        if (finished) {
            finishedBlock();
